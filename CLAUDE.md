...
1. **Check Current Task** → `list_tasks(filter_by="status", filter_value="todo")`
2. **Research for Task** → `search_code_examples()` + `perform_rag_query()`
3. **Implement the Task** → Write code based on research
4. **Update Task Status** → `update_task(task_id="...", status="doing" → "review" → "done")`
5. **Get Next Task** → `list_tasks(filter_by="status", filter_value="todo")`
6. **Repeat Cycle**
...
create_project(title="Existing Project Name", description="...")
...
list_tasks(filter_by="project", filter_value="[project_id]")
...
perform_rag_query(query="[technology] architecture patterns", match_count=5)

# Specific implementation guidance  
search_code_examples(query="[specific feature] implementation", match_count=3)
```
...
**1. Get Task Details:**
```python
get_task(task_id="[current_task_id]")
```
...
perform_rag_query(query="microservices vs monolith pros cons", match_count=5)

# Security considerations  
perform_rag_query(query="OAuth 2.0 PKCE flow implementation", match_count=3)

# Specific API usage
perform_rag_query(query="React useEffect cleanup function", match_count=2)

# Configuration & setup
perform_rag_query(query="Docker multi-stage build Node.js", match_count=3)

# Debugging & troubleshooting
perform_rag_query(query="TypeScript generic type inference error", match_count=2)
```
...
search_code_examples(query="React custom hook data fetching", match_count=3)

# For specific technical challenges
search_code_examples(query="PostgreSQL connection pooling Node.js", match_count=2)
```
...
2. Review project status: `list_tasks(filter_by="project", filter_value="...")`
